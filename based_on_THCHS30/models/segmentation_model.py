...
class SegmentationModel(object):
    ...
    def build_beam_search_predictor(self,
        ...
        with tf.variable_scope("decoder", reuse=reuse):
            return encoder_output,tf.nn.ctc_beam_search_decoder(
                tf.transpose(encoder_output, perm=[1, 0, 2]),
                frequencies_seq_len,
    ...
    def __build_encoder(self,
        ...
        with tf.variable_scope("speaker_embedding", reuse=reuse):
            ...
            speaker_embedding = tf.Variable(np.identity(self.num_speakers,dtype=np.float32))

            speaker_embedding_output = tf.nn.embedding_lookup(speaker_embedding, speaker_ids)
        with tf.variable_scope("encoder", reuse=reuse):

            speaker_embedding_projection1 = tf.layers.dense(
                speaker_embedding_output, self.model_parameters["conv_num_filters"],tf.nn.softsign,
                kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                bias_initializer=tf.zeros_initializer
            )
            speaker_embedding_projection2 = tf.layers.dense(
                speaker_embedding_output, self.model_parameters["num_bidirectional_units"],tf.nn.softsign,
                kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                bias_initializer=tf.zeros_initializer
            )#batch_size,37
            

            speaker_embedding_projection1 = tf.expand_dims(tf.expand_dims(
                speaker_embedding_projection1,
                1), 1)#batch_size,1,1,37

            speaker_embedding_projection1 = tf.tile(
                speaker_embedding_projection1, [1, tf.shape(frequencies)[1], tf.shape(frequencies)[2], 1]#batch_size,10,20,37
            )
                    ...
                    with tf.variable_scope("conv_layer_" + str(i)):
                        output = tf.nn.relu(
                            tf.contrib.layers.batch_norm(tf.layers.conv2d(
                                ...
                                kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                                bias_initializer=tf.zeros_initializer
                            ), scope='bn', is_training=is_training) * speaker_embedding_projection1 + output
                        )
            output=tf.nn.relu6(output, name=None);
            ...
            with tf.variable_scope("bidirectional_layers"):
                cells_fw = [
                    tf.nn.rnn_cell.GRUCell(self.model_parameters["num_bidirectional_units"],kernel_initializer=tf.truncated_normal_initializer(stddev=0.01),
                                           bias_initializer=tf.zeros_initializer)
                ...
                bidirectional_output, _, _ = tf.contrib.rnn.stack_bidirectional_dynamic_rnn(
                    ...
                    initial_states_fw=[speaker_embedding_projection2] * self.model_parameters["num_bidirectional_layers"],
                    initial_states_bw=[speaker_embedding_projection2] * self.model_parameters["num_bidirectional_layers"]
                )
