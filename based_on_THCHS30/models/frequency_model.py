...
class FrequencyModel(object):
    ...
    def __build_encoder(self,
        ...
        with tf.variable_scope("bidirectional_layers", reuse=reuse):
            ...
            )#2,381,num_bidirectional_units*2
        with tf.variable_scope("voiced_probability_model", reuse=reuse):
            voiced = tf.layers.dense(bidirectional, 2, tf.sigmoid)#2,381,2
        with tf.variable_scope("w", reuse=reuse):
            w = tf.layers.dense(bidirectional, 1, tf.sigmoid)#2,381,1
        ...
        f = tf.squeeze(w * f_gru + (1 - w) + f_conv)#2,381
    ...
    def build_train_operations(self,
        ...
        return voiced,f_zero,train_op, loss, global_step, summary
