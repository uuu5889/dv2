...
class GRUCell(RNNCell):
  ...
  def call(self, inputs, state):
    ...
    new_h = u * state + (1 - u) * c#u is z,state is s_t-1,c is h,inputs is x_t.r is r
    return new_h, new_h

#qrnn
class QRNNCell(RNNCell):
  """Gated Recurrent Unit cell (cf. http://arxiv.org/abs/1406.1078).

  Args:
    num_units: int, The number of units in the QRNN cell.
    activation: Nonlinearity to use.  Default: `tanh`.
    reuse: (optional) Python boolean describing whether to reuse variables
     in an existing scope.  If not `True`, and the existing scope already has
     the given variables, an error is raised.
  ...
  def __init__(self,
               ...
               kernel_initializer=None,
               bias_initializer=None):
    super(QRNNCell, self).__init__(_reuse=reuse)
    self._num_units = num_units
    self._activation = activation or math_ops.tanh
    self._kernel_initializer = kernel_initializer
    self._bias_initializer = bias_initializer
  ...
  def call(self, inputs, state):
    """Gated recurrent unit (QRNN) with nunits cells."""
    if self._gate_linear is None:
      bias_ones = self._bias_initializer
      if self._bias_initializer is None:
        bias_ones = init_ops.constant_initializer(1.0, dtype=inputs.dtype)
    ...
    if self._candidate_linear is None:
      with vs.variable_scope("candidate"):
        self._candidate_linear = _Linear(
            ...
            bias_initializer=self._bias_initializer,
            kernel_initializer=self._kernel_initializer)
    c = self._activation(self._candidate_linear([inputs, state]))
    new_h = u * state + (1 - u) * c
    new_z=new_h*r
    return new_z, new_z
...
class _Linear(object):
  ...
  def __init__(self,
    ...
    with vs.variable_scope(scope) as outer_scope:
      ...
      shape_w=self._weights.get_shape()
